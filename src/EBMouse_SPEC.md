...
- Reference: Toolbox — all 9 interactive guides built (toolbox complete; covers both RCT and observational SR/MA)
  - Core GRADE Quick Guide (`/toolbox/core-grade`): 8-tab page covering overview, NRSI/observational, imprecision (with interactive SVG number-line diagrams), inconsistency, risk of bias, indirectness, publication bias, and SoF tables. Based on BMJ 2025 Core GRADE Papers 1–7 + GRADE Guidelines 6. Each of the 5 rate-down domain tabs (Inconsistency, Imprecision, RoB, Indirectness, Pub Bias) follows a consistent competition-defense pattern: Core Concept callout at top, decision flowchart, domain-specific deep-dive accordions, 7-row Common Pitfalls table, and 5-row "vs Other Domains" comparison. Per-tab specifics: **Inconsistency** adds Heterogeneity vs Inconsistency distinction, three-visual-criteria with closing principle, I² two-pitfalls callout with Karlsen knee-arthroplasty worked example (Core GRADE 3 Fig 4), and ICEMAN subgroup credibility flow. **Imprecision** adds Binary-vs-Continuous framing, CI ratio quick rule per Core GRADE 2 Fig 4 (RR ≥3 / OR ≥2.5 → consider rate-down-2; otherwise compute OIS), and the Core GRADE 4 stance that OIS is a secondary check when CI clearly excludes the threshold. **Risk of Bias** adopts RoB 2's 5 modern domains directly as the primary framework (replacing the legacy RoB 1 list), with per-outcome RoB emphasis, leave-one-out sensitivity analysis, and outcome-objectivity moderating blinding requirement (Core GRADE 4). **Indirectness** uses a PICO 4-element comparison table with Setting marked as a supplementary practical consideration (not a formal Core GRADE category per Paper 5), surrogate-outcome warning, and explicit decision flowchart. **Publication Bias** splits Selective Outcome Reporting into Type 1 (entire outcome unreported → PB domain) vs Type 2 (estimate selection → RoB Domain 5) per Core GRADE 4, lists 4 assessment methods (funnel plot, ≥10-study statistical tests, comprehensive search with registries, trim-and-fill), and structures funnel-asymmetry alternative explanations with explicit attribution to Core GRADE 4 (3 alternatives) vs broader meta-analysis literature (2 more). **NRSI tab** covers starting point at Low (★★), NRSI tools (NOS, CLARITY, ROBINS-I), case-series rules including harms-only special case, direction-of-bias logic, rate-up criteria (RR>2/<0.5 → +1; RR>5/<0.2 → +2). Cross-references in RoB / Pub Bias / SoF tabs link back to NRSI tab.
